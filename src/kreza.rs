...
impl Kreza {
    ...
    pub fn evaluate(
        ...
        Evaluation {
            verdict,
            score: final_score,
            council_votes: llm_eval.council_votes,
            metrics,
            fitness_delta: delta,
        }
    }
    ...
    fn llm_evaluation_with_model(
        ...
        let context_hint = format!(
            ...
            sug.file_path, sug.reason, sug.objective, proposal.hypothesis.statement, // hypothesis.statement هو String، يمكن استخدامه مباشرة
        ...
        match serde_json::from_str::<KrezaResponse>(&response) {
            Ok(kr) => {
                ...
                Evaluation { verdict, score, council_votes: vec![], metrics, fitness_delta: delta }
            },
            Err(e) => Evaluation {
